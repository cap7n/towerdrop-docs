...
The tower is the thing you protect. Enemies walk to it, **climb its wall**, and attack it, digging in at a range of heights along the wall rather than only ever reaching the rim. If it falls, the run ends.
...
The tower is packed into its own `GameTower` scene with reparent-safe, group-based references; see [Technical Solutions](../tech/solutions.md) for why.

## The new modeler tower + brick shell

A new tower model was delivered by 3D modeler **Jennifer** and extracted into `res://Imported/NewTower/`. Its decorative **brick shell is rebuilt as a MultiMesh** (one draw call) from Jennifer's own geometry-node layout: 110 accent bricks.
...
This is the foundation for **visible tower damage**: enemies already dig in and attack at varied heights along the wall (that positional mechanic is live), but the tower doesn't yet show cracks/chipped bricks as it takes damage. See the [Backlog](../project/backlog.md) and [Asset Pipeline](../tech/asset-pipeline.md).

## Related

- [Enemies](enemies.md): how climbers reach and attack the wall.
