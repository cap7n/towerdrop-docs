...
**Roadmap markers (2026-07-24):** entries scheduled into the versioned [0.1x roadmap](roadmap.md) carry a **→ 0.xx** milestone marker; **→ parked (post-0.1x)** = a Bigger-bet outside the series.

---

## Playtest build v0.1 (prototype-era goal — superseded by the [0.1x roadmap](roadmap.md))
...
- <span class="pill todo">Todo</span> **→ 0.13** **Waves beatable to ≥10**: tune size/HP ramp and make sure defenses keep up.
- <span class="pill todo">Todo</span> **→ 0.12** **Item→effect coverage**: every buyable/usable item must actually do something (no dead items).

### Tier 2: onboarding

- <span class="pill todo">Todo</span> **→ 0.14** **Tutorial overhaul**: walk the actual systems (cart drop/pour + aimed throw, element trees, base + wizard shops, build/traps, tower ultimates), not just intro hints.
- <span class="pill todo">Todo</span> **→ 0.14** **Spellbook onboarding** — since unowned spells are hidden (2026-07-19), nothing teaches the book exists. Wanted: a tutorial beat when the FIRST ultimate is bought (point at the right-edge tab / auto-open the book on its new spell).
- <span class="pill todo">Todo</span> **→ 0.14** **Controls reference** card (move / drop / throw / shop keys), a pause-screen list at minimum.
- <span class="pill todo">Todo</span> **→ 0.14** **Quick Tutorial Screen** — a fast, single-screen quick-reference (the essentials: move / drop / throw / shop / repair) for players who skip the full guided mode.
- <span class="pill todo">Todo</span> **→ 0.14** **New-player tutorial** — the dedicated guided new-player tutorial mode (the revamp's [Track 1](revamp.md)): slow, its own thing, walks each system.
...
- <span class="pill wip">WIP</span> **→ 0.12** **Rock ultimate: Meteorite Shower** — v1 CASTABLE (2026-07-19): `MeteorShower` (Levels/Scripts) rains real rock DroppedItems over the incoming sector (near→fog band, sqrt-area spread, drip-spawned, live-item perf cap; all knobs @export — 100/s × 10 s start values need tuning). Wired as a 4th card in the test spellbook. Still open: the rock-tree node for it, wizard-buff inheritance (see Decisions), and the real spellbook home.
- <span class="pill idea">Idea</span> Do Fire / Electric / Lure want their own tower ultimates too? (6-element set)
- <span class="pill todo">Todo</span> **→ 0.12** Tower ultimates gated **deep in the wizard tree** rather than the always-on spellbook.

## Bigger features (later)

- <span class="pill wip">WIP</span> **→ 0.10** **Save system (run checkpoints)** — feasibility checked 2026-07-21: cleanly doable. **Phase 1 (the profile layer) BUILT 2026-07-24**: `SaveSystem` autoload (registered last; `user://profiles/index.cfg` registry + per-slot dirs, versioned `{version, data}` JSON with newer-version-loads-as-no-save, legacy `user://save_run.json` adoption, meta.json with defaults + old-meta key-merge, playtime banking with a last-flushed marker — flushed on game over / quit-to-menu / restart / window close; game over deletes the run file and keeps meta) + the menu **Profiles screen**: **three fixed cards** (Player 1/2/3 always exist — Yaro's call 2026-07-24, replacing the create/delete list) — click a card to select (gold border = active), name on top, the card BODY is reserved for per-profile stats at 0.11 (playtime shows small at the bottom already), inline Rename per card; the Profiles button reads "Profiles: name". Headless-tested. **Phase 2 (the run checkpoint) BUILT 2026-07-24 — the save system is feature-complete for 0.10**: `RunCheckpoint` (Levels/Scripts) gathers/applies the full state surface — Globals (gold, unlocks, per-item dicts), TAB-shop counts (level-absolute re-apply; base_max_hp + hire_wizard skipped by design), wizards (exact rim slots + element/tree/gold-sunk), artifacts (bag counts + placed relics re-mounted through the real setup path, Thick Bark / Crest Shield bonuses re-fire), tower HP raw-last (no double bonuses), per-brick armour (HP-authoritative), cart (items by resource path + selection) — and resumes in the saved wave's REST with the auto-next collapse suppressed once (full window to reorient). Saves at the REST **field-clear one-shot** + a re-save on quit-to-menu while resting (rest purchases persist; mid-assault quits keep the last wave's file). Menu **Continue (wave N)** button (per profile), **Play deletes the slot's run**, restart too; fixed in passing: artifacts no longer leak across runs (only game over used to clear the bag). 31-check headless round-trip passed (build a run on level_1 → save → fresh level via Continue → every surface byte-identical). **Coin-pile seeding (2026-07-24, Yaro's call):** the physical hoard isn't saved — on Continue the saved gold NUMBER is re-released onto the pile as synthetic already-settled coins (`CoinPile.seed_coins`): no physics, no gold grant (a real drop would double the count — `absorb()` is where gold is earned), each coin seated on the growing surface so the mound builds organically; spend-shrink works from the seeded base. Verified by test (seed 321 → exact number, shell, heightmap, spend → 221). Model: **checkpoint at REST** (between waves — no live enemies or in-flight items, so the transient mess never gets saved). Auto-save on entering rest + on quit-to-menu; delete the file on game over; menu gets a **Continue** button when a save exists. One `SaveSystem` autoload writing versioned JSON to `user://save_run.json`. The full state surface, all confirmed serializable:
    ...
    - **Per-profile extension + grounded plan (2026-07-24, Yaro):** decision — save is **per named profile / slot**, not one global file. Storage: `user://profiles/index.cfg` (registry: `{id, display_name, created, last_played}` + `active_id`) and `user://profiles/<id>/save_run.json` per slot. `id` is a generated slug so renaming never moves files; adopt any legacy `user://save_run.json` into a "Profile 1" slot on first launch. Nothing player-facing exists yet — the menu **Profiles** button is a stub (`start_menu.gd` `_on_profiles`), and the only `profile` in code is unrelated (wizard *profile cards*, FluidField perf profiling).
        - *Save trigger (precise):* do **not** save on `_enter_rest()` — that also fires on an assault *timeout* with a stuck straggler alive. Save on the field-clear gate already in `wave_manager.gd` REST (`_phase_timer <= 0.0 and get_alive_count() <= 0`), as a one-shot the first frame REST is confirmed clear, reset when the next assault begins. Guarantees "no enemy presence". Plus a save on quit-to-menu.
        - *SaveSystem autoload:* register **last** in `[autoload]` so every subsystem it reads is up. API: `list/create/rename/delete_profile`, `set_active/get_active_id`, `has_run` (drives Continue), `save_run`, `load_run`+`apply_run`, `delete_run`. Format `{version, data}`; unknown/newer version loads as "no save" rather than crashing.
        - *Artifact-surface correction:* the placed-artifact model in the bullet above (`{id, transform, linked_brick}`, `mounted=true`) is **not on the current game branch** — `ArtifactInventory` here is counts-based (`_collected` id→count + `bonus()`/`_apply_immediate()`). v1 saves just the `_collected` dict and restores by writing it back + recomputing bonuses; do **not** re-`grant()` (that re-runs buy-time side effects and stacks them). Bump `version` to add transforms if the placed rework lands.
        - *Meta layer (added 2026-07-24 — artifacts are meta-gated):* **→ 0.11** this settles the "cross-run meta progression (separate decision)" caveat above — decided, and it changes the layout: each profile gets a second file `user://profiles/<id>/meta.json` that is **never deleted on game over** (only `delete_profile` removes it, along with the whole slot dir). Contents: `playtime_total` (accumulated from `run_time` deltas, flushed at the same REST checkpoint + quit-to-menu + game over, with a last-flushed marker so restarts never double-count), `achievements` (id → progress/unlocked), `shards` (balance + lifetime earned; earn source TBD — see Meta-progression), `artifacts_unlocked` (the profile's pool). SaveSystem API grows `save_meta`/`load_meta` + `is_artifact_unlocked(id)`. **Gating point:** `ArtifactInventory.grant_random()` draws from the catalogue **filtered to `artifacts_unlocked`** (a locked id can never be granted); a loaded run's `_collected` is trusted as-is — ids collected legitimately stay even if the unlock table changes later. **Game over now does BOTH:** delete the run file AND flush meta — a lost run still banks playtime, shards, and achievement progress. Achievements v1 = a small tracker inside SaveSystem fed by existing signals (`kill_recorded`, `wave_started`, `gold_changed`, `hp_changed`); meta gets its own `version` field, migrated independently of the run format.
        - *Load/apply order:* Continue sets active profile + "load pending", loads `level_1`; `wave_manager._ready()` already `call_deferred("_begin_run")` is the fork → branch to `apply_run()` (deferred, after tree ready): Globals → tower HP (raw) → shop `_apply` replay → wizards (place N then push `_tree_levels`/`element`) → artifacts `_collected` → cart → WaveManager resumes at `Phase.REST` of the saved wave, re-emitting `gold_changed`/`hp_changed`/unlock signals. **Play** stays new-run + `delete_run` for that slot; **game over** deletes only the *active* profile's run file.
        - *Menu:* Profiles screen (list/create/rename/delete/pick active) replaces the stub; **Continue** shown only when `has_run(active)`. Menu-tower-wears-artifacts stays out of scope (placed artifacts absent on this branch).
- <span class="pill todo">Todo</span> **→ parked (post-0.1x)** **Menu tower wears your saved artifacts** (parked idea, Cap7n 2026-07-21) — while the cart orbits the menu tower, the player sees THEIR artifacts from the save file mounted on it, scrolling by. Implementation sketch: the save file already stores placed artifacts as `{id, transform, linked_brick}` relative to the tower axis — `menu_world.tscn` (the 3D backdrop) reads `user://save_run.json` on menu load and instances each artifact's **visual scene only** (no `mounted=true`, no effects — the autoload level-guard already keeps gameplay dormant in menus) at the saved transform on the menu tower. Needs: the menu tower to share the game tower's dimensions (it does — same model), a small "spawn visuals from save" helper in the menu world script, and skipping brick-linked artifacts if the menu tower has no brick shell (or just mounting them at the transform anyway — it's cosmetic). Depends on: the save system above.
...
- <span class="pill todo">Todo</span> **→ parked (post-0.1x)** **Buildable defenses**: satellite towers + traps (TAB-shop → build tray → click-to-place). Iter-1 exists but free / no purchase.
- <span class="pill todo">Todo</span> **→ parked (post-0.1x)** **Traps**: more placeable ground hazards (caltrops / grinder / conveyor / chain); new plan: buy at a wizard then throw into place (reuse aimed-throw).
- <span class="pill wip">WIP</span> **→ 0.13** **More enemy types**: scripts done (thin `extends Enemy` subclasses); each needs a model + scene:
    ...
    - <span class="pill todo">Todo</span> **→ 0.12** Per-element resistance/weakness system (quick-test version exists).
- <span class="pill todo">Todo</span> **→ parked (post-0.1x)** **Flying enemy (moth)**: parked on the anti-air question; answer agreed: aimed-throw = skill-shot AA + an AA tower upgrade. Build after aimed-throw feel is locked. *(Related: the **thief flyer** idea — steals gold/artifacts and flees — is an [open question in Decisions](decisions.md).)*
- <span class="pill wip">WIP</span> **→ 0.13** **Enemy component system**: built (`EnemyLocomotion` + `EnemyAttack` + `EnemyAbility[]`), opt-in inside `enemy.gd`. TODO: A/B `Spider_v2` vs `Spider`, port Snail, migrate the 5 subclasses, then delete the legacy inline path.
...
- <span class="pill todo">Todo</span> **→ 0.12** **Electric + Oil/wet = arc conductor**: chain lightning jumps further to oiled neighbors.
...
- <span class="pill todo">Todo</span> **→ 0.13** **New mechanic**: "only THIS element penetrates" layer (inverse of immunity). Visual not figured out.
...
- <span class="pill todo">Todo</span> **→ 0.14** **Improve the onboarding tutorial** (see the Tutorial overhaul items).
...
- <span class="pill wip">WIP</span> **→ 0.13** **Kill the snowball**: went unkillable past wave 10 (spikes insta-kill + regen heals all back). Done: removed spikes / poisoned spikes / HP regen from the TAB tree (systems kept). Next: budget wave generator, late-game tuning.
- <span class="pill todo">Todo</span> **→ 0.12** **Spikes → Rock-wizard ULTIMATE** (decided 2026-07-12: cast-for-gold with a duration, like Oil/Frost/Slime — not a permanent upgrade). Folds into the ultimates-gating pass; `base_spikes`/`poisoned_spikes` tree nodes + the SpikeRing mechanics (still in shop_graph) are the ingredients.
- <span class="pill todo">Todo</span> **→ 0.12** **HP regen → upgradeable spell**, not a forever passive. *(Or superseded by the new REPAIR-hammer idea, see 2026-07-12 playtest notes.)*
- <span class="pill todo">Todo</span> **→ 0.13** **Frost is underpowered**: concept: snow-storm on hit (slow + DoT AoE), "avalanche" upgrade.
- <span class="pill todo">Todo</span> **→ 0.13** **Fire & lightning too cheap vs rock**: element cost rebalance.
- <span class="pill todo">Todo</span> **→ 0.12** **BUG: lightning's spread fires on ROCK kills**: element effect bleeding across equipped wizards.
- <span class="pill todo">Todo</span> **→ 0.13** **Roller (pillbug) aiming is un-fun**: make the path visible or the orbit predictable.
...
- <span class="pill todo">Todo</span> **→ 0.16** **Unique beam VFX per element** — all 6 `element:beam` artifacts currently share one pack scene (`laser_vfx_01`), differing only by recolor, so a fire beam and a frost beam look identical apart from hue. Give each element its own beam identity: pick a distinct Binbun `laser_vfx_0X` / `beam_vfx_0X` preset per element (chunky vs thin vs jagged), and/or per-element impact behaviour (fire beam ignites, frost beam hard-slows, electric arcs). One-line preload swap per element; wire via the catalogue so each `*_beam` entry names its own scene.

## Art / style

- <span class="pill todo">Todo</span> **→ 0.17** **Stylized art pass**: settle a consistent look once real models exist. Parked toon-shader tech in `VFX/Shaders/toon*.gdshader`. No outlines (faceted / soft-matte).

## Polish / quick wins

- <span class="pill todo">Todo</span> **→ 0.16** **VFX for aim line** — the cannon-shot trajectory line (hold right-click) is currently a plain translucent ribbon that reads like a debug draw. Give it a proper magical treatment: shader with flow/scroll along the path, soft glow, maybe dashes or sparkle drift; keep the purple = clear / red = blocked language. Lives in `cart.gd` (`_make_aim_line` / `_draw_aim_line`, camera-facing ImmediateMesh ribbon, `aim_line_width` export).
- <span class="pill todo">Todo</span> **→ 0.16** **Per-wizard damage tracking** — record damage dealt per wizard (attribute each summon's hits to its wizard) and show it on the shop's Stats screen. The Stats screen itself (gold invested + owned upgrades per wizard) shipped 2026-07-19.
- <span class="pill todo">Todo</span> **→ 0.17** **Goblin wizard: more animations** — a real Idle (a procedural one already ships unused inside `Goblin.glb`), hat-tip on purchase, ear wiggle (ear bones exist), a "ta-dah" when an item finishes. The archer model in the same folder can reuse the whole rig pipeline.
- <span class="pill todo">Todo</span> **→ 0.16** **Grass trample: smooth the pop-back** — blades still rise a touch fast right behind a walker (the just-left spot only ever got the stamp's soft skirt). Ideas: max-blend stamping, ease the shader response (str²), widen the flat core.
...
- <span class="pill todo">Todo</span> **→ 0.13** Tuning pass on the 6 element trees + TAB base tree + lure feel.
...
- <span class="pill todo">Todo</span> **→ pre-public gate (0.18+)** **Git HISTORY still contains the RBL files** — the repo must stay **private** until history is purged (`git filter-repo` on `Imported/RollCall/`) or accept-and-never-open-source. Do the purge before any public/source release.
