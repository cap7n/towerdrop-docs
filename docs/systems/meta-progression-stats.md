...
| **Repair rate** | `hp_per_repair` / `repair_interval` | — (doesn't exist yet) | 🔴 | ⏳ **Blocked on the repair rework** ([high-prio backlog](../project/backlog.md)): repair is currently one-click-full-heal, so there is no rate to upgrade. Once it's a hold-to-repair tick (~0.3 s start), *both* fields become prime nodes — repair speed is a defensive stat with no meta representation today. |
