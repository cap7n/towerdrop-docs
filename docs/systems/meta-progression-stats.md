...
| **HP per repair click** | `hp_per_repair` | 1.0 *(decided, not built)* | 🔴 | ⏳ **Blocked on the repair rework** ([high-prio backlog](../project/backlog.md)): repair is one-click-full-heal today, so there is no rate to upgrade. **Decided 2026-07-29:** a click restores 1 HP, making this the base a node multiplies. Repair speed is a defensive stat with no meta representation at all right now. |
| **Repair tick interval** | `repair_interval` | ~0.3 s *(follow-up)* | 🔴 | The hold-to-repair layer over the same base. Second node once it exists. |
